...
if vim.fn.has 'win32' == 1 and not is_ssh then
  vim.g.clipboard = require 'custom.clipboard'
elseif is_ssh then
  -- SSH session - use built-in OSC 52
  vim.g.clipboard = {
    name = 'OSC 52',
    copy = {
...
-- Alt + arrows: window navigation that also works from terminal mode (e.g. inside Claude Code).
for key, dir in pairs { Left = 'h', Right = 'l', Down = 'j', Up = 'k' } do
  local lhs = '<M-' .. key .. '>'
  local rhs = '<Cmd>wincmd ' .. dir .. '<CR>'
  vim.keymap.set({ 'n', 't' }, lhs, rhs, { desc = 'Move focus ' .. key:lower() })
end
...
--
-- [[ Custom keymaps import]]
require 'custom.keymaps'
--
-- [[ Custom Options import ]]
require 'custom.opts'

-- [[ Custom Highlight fixups ]]
    ...
    config = function()
      ...
      vim.api.nvim_create_autocmd('LspAttach', {
        ...
        callback = function(event)
          ...
          -- NOTE: Replaced with snacks plugin
  ...
  { -- Autocompletion
    ...
    opts = {
      keymap = {
        ...
        -- 'Tab' to cycle DOWN through the list
        ['<Tab>'] = { 'select_next', 'snippet_forward', 'fallback' },

        -- 'Shift-Tab' to cycle UP through the list
